...
export default function CreateChapter(props) {
  ...
  useEffect(() => {
    if (props.link !== window.location.pathname)
      setCurrentPageStyle("not-current-page");
  ...
  return (
    <Link
      ...
      to={props.link}
      className="text-custom-light-cream"
    >
      <div
        className={`${currentPageStyle} text-info  pb-[15px] pt-[15px] p-[10px] text-[1rem] max-w-[100px] transition-all duration-100`}
      >
        {props.name}
      </div>
    </Link>
